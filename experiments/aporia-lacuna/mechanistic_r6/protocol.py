...
DEVELOPMENT_SEEDS = tuple(range(109001, 109049))
CONFIRMATION_SEEDS = tuple(range(110001, 110049))
DEVELOPMENT_SYNTHETIC_SEEDS = tuple(range(111001, 111129))
CONFIRMATION_SYNTHETIC_SEEDS = tuple(range(112001, 112129))
...
class LunaProposalProvider:
    ...
    def generate(self, arguments: dict[str, Any]) -> RawProposal:
        ...
        if set(value) != {"proposal"} or value.get("proposal") not in {"A", "B"}:
            raise RuntimeError("mechanistic_r6_model_response_invalid")
...
def run_protocol(
    ...
    if phase not in {"development", "confirmation"}:
        raise ValueError("mechanistic_r6_phase_invalid")
    if len(factor_seeds) != 48 or len(synthetic_seeds) != 128:
        raise ValueError("mechanistic_r6_seed_count_invalid")
    power = simulate_power(48, 0.55, 0.90, 4000, 108991)
...
def run_factorial(seeds: tuple[int, ...], provider: ProposalProvider) -> FactorialReport:
    ...
    for seed in seeds:
        ...
        if not raw.response_id or raw.response_id in response_ids or raw.model != "gpt-5.6-luna":
            raise RuntimeError("mechanistic_r6_provider_identity_invalid")
        ...
        if not aporia.install(authority, scar, episode, parent, 1):
            raise RuntimeError("mechanistic_r6_authentic_scar_rejected")
...
def run_provenance(
    ...
    for index, seed in enumerate(seeds):
        ...
        if authentic_gateway is None:
            raise RuntimeError("mechanistic_r6_authentic_gateway_missing")
        ...
        if not graft_gateway.install(authority, graft, episode, parent, 1):
            raise RuntimeError("mechanistic_r6_authorized_graft_rejected")
...
def _decision_context(seed: int, arm: str) -> dict[str, Any]:
    if arm not in {"aporia", "zombie_plus"}:
        raise ValueError("mechanistic_r6_arm_invalid")
    return {
