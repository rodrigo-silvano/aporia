...
def main() -> int:
    ...
    if os.environ.get("APP_ENV", "").strip().lower() != "staging":
        raise RuntimeError("mechanistic_r7_staging_required")
    if not arguments.output or not arguments.execution_ledger or not arguments.expected_commitment:
        raise RuntimeError("mechanistic_r7_confirmatory_requirements_missing")
    ...
    if not api_key:
        raise RuntimeError("mechanistic_r7_openai_key_required")
    ...
    try:
        ...
        receipt = {
            "protocol_key": "aporia_mechanistic_r7",
            "execution_count": 1,
            "environment": "staging",
            "status": "completed",
            "rerun_allowed": False,
...
def _payload(report: dict[str, object]) -> dict[str, object]:
    gates = (
        ("information_isomorphism", report["information_parity_by_construction"]),
        ("operator_crossover", report["operator_crossover"]),
        ("binding_dependence", report["binding_dependence"]),
        ("representation_robustness", report["representation_robustness"]),
        ("trajectory_dependence", report["trajectory_dependence"]),
    )
    first_failed = next((name for name, state in gates if state != "PASS"), None)
    return {
        "protocol_key": "aporia_mechanistic_r7",
        "phase": report["phase"],
        "analysis_unit": "synthetic_world_seed_block",
        "report": report,
        "technical_execution_complete": True,
        "technical_gates_passed": first_failed is None,
        "confirmatory_execution_eligible": first_failed is None,
        "first_failed_gate": first_failed,
        "e_forecast": "ACCRUING",
        "ecological_generalization": "BLOCKED_BY_REAL_DATA",
        "g8": "SEALED",
        "e_policy": "NOT_ELIGIBLE",
        "policy_eligible": False,
        "scientific_claim": "information_isomorphic_functional_mechanism_only",
        "production_eligible": False,
...
def _credentials(path: Path | None) -> tuple[str, str]:
    ...
    if path is not None:
        ...
        if path.is_symlink() or not resolved.is_file() or resolved.stat().st_size > 131072:
            raise RuntimeError("mechanistic_r7_env_unavailable")
...
def _validated_base_url(value: str) -> str:
    ...
    if any((parsed.scheme != "https", parsed.hostname != "api.openai.com", parsed.username, parsed.password, parsed.query, parsed.fragment)):
        raise RuntimeError("mechanistic_r7_base_url_invalid")
...
def _confirmatory_paths(output: Path, ledger: Path) -> tuple[Path, Path]:
    ...
    if output_parent != ledger_parent:
        raise RuntimeError("mechanistic_r7_confirmatory_storage_mismatch")
    output_parent.mkdir(parents=True, exist_ok=True)
    if output_parent.is_symlink() or not output_parent.is_dir():
        raise RuntimeError("mechanistic_r7_confirmatory_storage_invalid")
...
def _reserve(path: Path) -> int:
    ...
    try:
        ...
        raise RuntimeError("mechanistic_r7_confirmatory_already_executed") from exception
