...
DEVELOPMENT_SEEDS = tuple(range(62001, 62257))
CONFIRMATION_SEEDS = tuple(range(63001, 63257))
...
def main() -> int:
    ...
    if arguments.mode == "confirmatory":
        if os.environ.get("APP_ENV", "").strip().lower() != "staging":
            raise RuntimeError("discrimination_r3_staging_required")
        if not arguments.output or not arguments.execution_ledger:
            raise RuntimeError("discrimination_r3_confirmatory_paths_required")
        ...
        if output.exists() or output.is_symlink() or ledger.exists() or ledger.is_symlink() or output.parent != ledger.parent:
            raise RuntimeError("discrimination_r3_confirmatory_already_executed")
    ...
    payload = {
        "protocol_key": "aporia_discrimination_r3",
        ...
        "scientific_claim": "path_dependent_computational_identity_discriminative_endpoints" if first_failed is None else "discriminative_effect_not_demonstrated",
