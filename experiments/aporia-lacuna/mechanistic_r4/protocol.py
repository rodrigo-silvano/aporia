...
DEVELOPMENT_SEEDS = tuple(range(85001, 85049))
CONFIRMATION_SEEDS = tuple(range(86001, 86049))
DEVELOPMENT_SYNTHETIC_SEEDS = tuple(range(87001, 87129))
CONFIRMATION_SYNTHETIC_SEEDS = tuple(range(88001, 88129))
...
class LunaProposalProvider:
    ...
    def generate(self, arguments: dict[str, Any]) -> RawProposal:
        ...
        if set(value) != {"proposal"} or value.get("proposal") not in {"A", "B"}:
            raise RuntimeError("mechanistic_r4_model_response_invalid")
        usage = _item(response, "usage", {}) or {}
        return RawProposal(
            str(value["proposal"]),
            str(_item(response, "id", "") or ""),
            str(_item(response, "model", "") or "gpt-5.6-luna"),
            output,
...
def run_protocol(
    ...
    if phase not in {"development", "confirmation"}:
        raise ValueError("mechanistic_r4_phase_invalid")
    if len(factor_seeds) != 48 or len(synthetic_seeds) != 128:
        raise ValueError("mechanistic_r4_seed_count_invalid")
    power = simulate_power(48, 0.55, 0.90, 4000, 84991)
...
def run_factorial(seeds: tuple[int, ...], provider: ProposalProvider) -> FactorialReport:
    authority = ScarAuthority("mechanistic-r4-factorial-authority")
    frozen = []
    raw_correct = 0
    aporia_correct = 0
    zombie_correct = 0
    ...
    for seed in seeds:
        ...
        prompt = canonical({
            "abstract_case": commitment(f"mechanistic-r4-visible|{seed}"),
            "visible_signal": visible_signal,
            "allowed_proposals": ["A", "B"],
        })
        arguments = request_arguments(prompt)
        decision_context = _decision_context(seed)
        ...
        if not raw.response_id or raw.model != "gpt-5.6-luna":
            raise RuntimeError("mechanistic_r4_provider_identity_invalid")
        ...
        if not aporia.install(authority, scar, episode, parent, 1):
            raise RuntimeError("mechanistic_r4_authentic_scar_rejected")
        ...
        request_matches += int(integrity["model_request_sha256"] == integrity["model_request_sha256"])
        ...
        frozen.append(FrozenResponse(
            ...
            integrity["prompt_wire_sha256"],
            integrity["decision_context_sha256"],
...
def run_provenance(
    ...
    authority = ScarAuthority("mechanistic-r4-provenance-authority")
    ...
    for index, seed in enumerate(seeds):
        ...
        if authentic_gateway is None:
            raise RuntimeError("mechanistic_r4_authentic_gateway_missing")
        ...
        if not graft_gateway.install(authority, graft, episode, parent, 1):
            raise RuntimeError("mechanistic_r4_authorized_graft_rejected")
    ...
    for length in TRAJECTORY_LENGTHS:
        for attack in LongitudinalPoisonGuard.ATTACKS:
            for seed in seeds[:16]:
                guard = LongitudinalPoisonGuard(49, f"poison-r4-{seed}")
                for episode in range(length):
                    guard.memory.append({
                        "source": "verified_outcome",
                        "episode_commitment": commitment(f"benign|{seed}|{episode}"),
                    })
                ...
                persisted_attack_fields += sum(
                    ...
                    for item in guard.memory
                )
                cross_tenant += int(attack == "cross_tenant_identity" and len(guard.memory) != length)
...
def _decision_context(seed: int) -> dict[str, Any]:
    return {
        ...
        "policy_version": "mechanistic-r4-policy-v1",
        "ontology_version": "ontology-v1",
        "hirt_version": "hirt-v1",
        "effect_registry_version": "effects-v1",
        "gateway_version": "mechanistic-r4",
