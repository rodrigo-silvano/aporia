...
class ScarAuthority:
    ...
    def authorized_graft(self, scar: Scar, lineage_ref: str, agent_ref: str, time_index: int) -> Scar:
        if not self.verify(
            ...
            raise ValueError("mechanistic_r5_graft_source_invalid")
        if time_index <= scar.time_index:
            raise ValueError("mechanistic_r5_graft_time_invalid")
        return self.issue(
            lineage_ref,
            agent_ref,
            scar.episode_ref,
...
class HistoricalBeliefLedger:
    ...
    def record(self, recorded_at: int, evidence_available_at: int, belief: str, evidence: str) -> BeliefRevision:
        if recorded_at < 0 or evidence_available_at < 0 or evidence_available_at > recorded_at:
            raise ValueError("mechanistic_r5_revision_time_invalid")
        if self.revisions and recorded_at <= self.revisions[-1].recorded_at:
            raise ValueError("mechanistic_r5_revision_order_invalid")
...
class LongitudinalPoisonGuard:
    ...
    def ingest(self, attack_class: str, payload: str, source_tenant_id: int) -> bool:
        if attack_class not in self.ATTACKS:
            raise ValueError("mechanistic_r5_attack_unknown")
        tenant_ref = hmac.new(
    ...
    def record_verified_outcome(self, episode_commitment: str) -> None:
        if len(episode_commitment) != 64:
            raise ValueError("mechanistic_r5_outcome_commitment_invalid")
        signature = hmac.new(
            self.secret.encode("utf-8"),
            f"outcome|{self.tenant_id}|{episode_commitment}".encode("utf-8"),
            hashlib.sha256,
        ).hexdigest()
        self._memory.append({
    ...
    def memory_entries(self) -> tuple[dict[str, str], ...]:
    ...
    def clean(self) -> int:
        ...
        for item in self._memory:
            commitment_value = item.get("episode_commitment", "")
            expected = hmac.new(
                self.secret.encode("utf-8"),
                f"outcome|{self.tenant_id}|{commitment_value}".encode("utf-8"),
                hashlib.sha256,
            ).hexdigest()
            valid = all((
                item.get("source") == "verified_outcome",
                item.get("tenant_id") == str(self.tenant_id),
                len(commitment_value) == 64,
                hmac.compare_digest(item.get("signature", ""), expected),
            ))
            if valid:
