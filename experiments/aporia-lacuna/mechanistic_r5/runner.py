...
def main() -> int:
    ...
    if os.environ.get("APP_ENV", "").strip().lower() != "staging":
        raise RuntimeError("mechanistic_r5_staging_required")
    if not arguments.output or not arguments.execution_ledger or not arguments.expected_commitment:
        raise RuntimeError("mechanistic_r5_confirmatory_requirements_missing")
    ...
    if not api_key:
        raise RuntimeError("mechanistic_r5_openai_key_required")
    validated_base_url = _validated_base_url(base_url)
    ledger_descriptor = _reserve(ledger)
    output_descriptor = -1
    provider: LunaProposalProvider | None = None
    ...
    try:
        ...
        receipt = {
            "protocol_key": "aporia_mechanistic_r5",
            "execution_count": 1,
            "environment": "staging",
            "status": "completed",
            "rerun_allowed": False,
        ...
        receipt = {
            ...
            "status": "invalidated",
            "rerun_allowed": False,
            "model_calls": len(provider.calls) if provider is not None else 0,
...
def _credentials(path: Path | None) -> tuple[str, str]:
    ...
    if path is not None:
        ...
        if path.is_symlink() or not resolved.is_file() or resolved.stat().st_size > 131072:
            raise RuntimeError("mechanistic_r5_env_unavailable")
...
def _validated_base_url(value: str) -> str:
    ...
    if any((parsed.scheme != "https", parsed.hostname != "api.openai.com", parsed.username, parsed.password, parsed.query, parsed.fragment)):
        raise RuntimeError("mechanistic_r5_base_url_invalid")
...
def _confirmatory_paths(output: Path, ledger: Path) -> tuple[Path, Path]:
    ...
    if output_parent != ledger_parent:
        raise RuntimeError("mechanistic_r5_confirmatory_storage_mismatch")
    output_parent.mkdir(parents=True, exist_ok=True)
    if output_parent.is_symlink() or not output_parent.is_dir():
        raise RuntimeError("mechanistic_r5_confirmatory_storage_invalid")
...
def _reserve(path: Path) -> int:
    ...
    try:
        ...
        raise RuntimeError("mechanistic_r5_confirmatory_already_executed") from exception
