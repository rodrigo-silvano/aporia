...
DEVELOPMENT_SEEDS = tuple(range(99001, 99257))
CONFIRMATION_SEEDS = tuple(range(99301, 99557))
...
def run_protocol(phase: str, seeds: tuple[int, ...]) -> OperationalReport:
    if phase not in {"development", "confirmation"} or len(seeds) != 256:
        raise ValueError("operational_r4_protocol_invalid")
...
def _worker_restart() -> tuple[Lifecycle, int, int]:
    ...
    restored = Lifecycle.restore(lifecycle.snapshot(), lifecycle.turn_ref, lifecycle.tenant_id)
...
def _repeated_revocation() -> tuple[Lifecycle, int, int]:
    ...
    try:
        ...
        if str(exception) != "operational_r4_revoked":
            raise
    return lifecycle, 0, 0
