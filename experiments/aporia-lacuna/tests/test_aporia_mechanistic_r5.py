...
from mechanistic_r5.mechanisms import ScarAuthority
from mechanistic_r5.protocol import (
    DEVELOPMENT_SEEDS,
...
from mechanistic_r5.runner import _confirmatory_paths, _reserve, _validated_base_url
from mechanistic_r5.sealing import assert_current_seal, code_commitment
...
def test_seal_and_power_are_preserved() -> None:
    ...
    assert simulate_power(48, 0.55, 0.90, 4000, 94991) >= 0.8
...
def test_confirmatory_paths_require_fresh_sibling_files(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert str(exception) == "mechanistic_r5_confirmatory_already_executed"
    else:
        raise AssertionError("mechanistic_r5_freshness_guard_missing")
...
def test_custom_provider_endpoint_must_be_official_https() -> None:
    ...
    for value in ("http://api.openai.com/v1", "https://example.com/v1", "https://api.openai.com/v1?x=1"):
        try:
            ...
            assert str(exception) == "mechanistic_r5_base_url_invalid"
        else:
            raise AssertionError("mechanistic_r5_base_url_guard_missing")


def test_authorized_graft_rejects_counterfeit_source() -> None:
    authority = ScarAuthority("r5-test-authority")
    ...
    try:
        ...
        assert str(exception) == "mechanistic_r5_graft_source_invalid"
    else:
        raise AssertionError("mechanistic_r5_graft_source_guard_missing")
