...
PROTOCOL_ID = "APORIA_PRODUCT_QUALIFICATION_RC4_V1"
SEED = 826_249_917
CREATED_AT = "2026-08-25T00:26:23Z"
TASK_INDEX_BASE = 40_000
CATEGORY_COUNTS = {
    "continuity": 60,
...
def sequences() -> list[dict[str, Any]]:
    ...
    for index in range(SEQUENCE_COUNT):
        ...
        sequence_ref = f"pqs4-{index + 1:03d}"
        turns = [
...
def verify() -> None:
    ...
    assert all(item["task_id"].startswith("pq4-") for item in corpus["tasks"]), "task_id_invalid"
    assert all(item["sequence_id"].startswith("pqs4-") for item in corpus["sequences"]), "sequence_id_invalid"
    assert all(
