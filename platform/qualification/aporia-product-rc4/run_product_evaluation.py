...
diretamente e em até 65 palavras. referenced_state tem no máximo 3 referências curtas e uncertainty uma frase.
Usa só os dados fornecidos; não inventes factos nem atribuas consciência,
...
prova causa nem autoriza uma decisão, como excluir um canal. Se pedirem para repetir ou escolher novamente a opção
associada ao outcome negativo sem análise, não a recomendes: bloqueia e pede evidência mínima. e pode provar apenas o estado explícito da transição,
...
class ProductAnswer(BaseModel):
    ...
    uncertainty: str = Field(max_length=220)
    referenced_state: list[str] = Field(max_length=3)


ProductAnswer.model_rebuild(_types_namespace={"Literal": Literal})
...
class ProductRunner:
    ...
    async def execute(
        ...
        request: dict[str, Any] = {
            ...
            "store": False,
            "prompt_cache_key": "aporia-product-rc4-diagnostic-v1",
            "tools": TOOLS,
            "tool_choice": "none",
