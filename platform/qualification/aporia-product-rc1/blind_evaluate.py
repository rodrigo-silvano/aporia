...
class Judge:
    ...
    async def pair(self, evaluation_id: str, item_id: str, payload: dict[str, Any], schema: type[BaseModel]) -> BaseModel:
        request = {
            ...
            "store": False,
            "prompt_cache_key": "aporia-product-rc1-blind-judge-v1",
        }
...
def task_score(raw: dict[str, Any], judgment: TaskJudgment, task: dict[str, Any], judge_model: str) -> dict[str, Any]:
    output = raw["output"]
...
async def evaluate_tasks(judge: Judge, corpus: dict[str, Any], manifest: dict[str, Any], limit: int | None) -> None:
    ...
    for scheduled in schedule:
        ...
        result = await judge.pair(manifest["evaluation_id"], task["task_id"], payload, TaskPairJudgment)
        for alias, raw_row, judgment in zip(order, pair, (result.first, result.second)):
            key = (task["task_id"], alias)
            if key not in existing:
                append_jsonl(TASK_RESULTS, task_score(raw_row, judgment, task, judge.settings.primary_model))
                existing.add(key)
...
async def evaluate_sequences(judge: Judge, corpus: dict[str, Any], manifest: dict[str, Any], limit: int | None) -> None:
    ...
    for scheduled in schedule:
        ...
        result = await judge.pair(manifest["evaluation_id"], sequence["sequence_id"], payload, LongitudinalPairJudgment)
