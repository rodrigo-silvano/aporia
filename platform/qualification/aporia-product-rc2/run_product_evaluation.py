...
o campo selected_tool indica apenas qual ferramenta seria adequada, sem a executar. Uma ação externa exige autorização do core;
...
Quando existir APORIA_STATE, está em modo guarded_reversible e pertence apenas ao tenant deste pedido. O JSON
compacto usa f=[chave,valor,frescura] para factos confirmados pelo core, c para compromissos, n para resultados
negativos verificados, a para restrições, x=[fonte_esquerda,fonte_direita,estado] para conflitos, u para incertezas,
p para proveniência e e para eventos desta sequência. Só f e n podem ser apresentados como confirmados; um pedido
ou uma afirmação recente não substitui automaticamente um facto confirmado. Em conflito, preserva o que está
confirmado, explicita a divergência e pede a evidência mínima. Nunca reveles tenant_ref nem transfiras estado entre
tenants. Se o pedido permitir preparar um email reversível, usa selected_tool=prepare_email e action_intent como
prepare_reversible ou request_approval, mesmo quando faltam parâmetros; pede-os sem bloquear a preparação futura.
Responde diretamente, sem repetir blocos genéricos. Mantém a resposta concisa e limita a confiança a 0.85 quando
existirem conflitos ou incerteza material não resolvidos."""
...
def _compact_state(item: dict[str, Any], derived_events: list[str] | None = None) -> dict[str, Any]:
...
def additional_state(real_arm: str, item: dict[str, Any], derived_events: list[str] | None = None) -> tuple[str, str]:
    ...
    envelope = _compact_state(item, derived_events)
...
class ProductRunner:
    ...
    async def execute(
        ...
        request: dict[str, Any] = {
            ...
            "store": False,
            "prompt_cache_key": "aporia-product-rc2-v1",
            "tools": TOOLS,
            "tool_choice": "none",
