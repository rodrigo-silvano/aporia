...
def main() -> None:
    ...
    operator_id = os.environ.get("APORIA_OPERATOR_ADMIN_ID", os.environ.get("PLS_OPERATOR_ADMIN_ID", "")).strip()
    if not operator_id.isdigit() or int(operator_id) < 1:
        raise SystemExit("APORIA_OPERATOR_ADMIN_ID_must_identify_active_admin")
