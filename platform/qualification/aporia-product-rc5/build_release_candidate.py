...
def validate_evidence(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    for name in sorted(paths):
        ...
        if payload.get("status") not in REQUIRED_EVIDENCE[name]:
        ...
        qualification_exception = None
        if name == "soak" and payload.get("status") == "PASS_WITH_EXCEPTION":
            qualification_exception = payload.get("qualification_exception")
            if not isinstance(qualification_exception, dict) \
                or qualification_exception.get("id") != "fresh_rc17_two_hour_soak_not_repeated" \
                or qualification_exception.get("accepted_by") != "explicit_user_direction" \
                or payload.get("fresh_uninterrupted_two_hour_soak") is not False \
                or payload.get("phase") != "combined_soak_delta" \
                or (payload.get("rc15_long_window") or {}).get("status") != "FAIL" \
                or (payload.get("rc16_targeted_delta") or {}).get("status") != "PASS" \
                or (payload.get("rc17_php_only_delta") or {}).get("agent_runtime_changed_from_rc16") is not False \
                or not payload.get("residual_limitations"):
                raise RuntimeError("rc_soak_exception_invalid")
        result[name] = {
        ...
        if qualification_exception is not None:
            result[name]["qualification_exception"] = qualification_exception
    return result
...
def validate_git_for_seal(release_commit: str | None = None) -> tuple[str, str]:
    ...
    generated_outputs = {
        str((HERE / "release-candidate-manifest.json").relative_to(ROOT)),
        str((HERE / "release-candidate-sbom.json").relative_to(ROOT)),
    }
    tracked_changes = set(filter(None, git("diff", "--name-only").splitlines()))
    if tracked_changes - generated_outputs:
    ...
    if release_commit is not None:
        if re.fullmatch(r"[0-9a-f]{40}", release_commit) is None \
            or git("cat-file", "-t", release_commit, check=False) != "commit":
            raise RuntimeError("rc_release_commit_invalid")
        if subprocess.run(
            ["git", "merge-base", "--is-ancestor", release_commit, "HEAD"],
            cwd=ROOT, check=False
        ).returncode != 0:
            raise RuntimeError("rc_release_commit_not_ancestor")
        qualification_prefix = str(HERE.relative_to(ROOT)) + "/"
        changed_after_release = git("diff", "--name-only", f"{release_commit}..HEAD")
        if any(
            path and not path.startswith(qualification_prefix)
            for path in changed_after_release.splitlines()
        ):
            raise RuntimeError("rc_release_commit_runtime_drift")
...
def main() -> None:
    ...
    branch, commit = validate_git_for_seal(args.release_commit)
    ...
    qualification_exceptions = [
        value["qualification_exception"]
        for value in evidence.values()
        if "qualification_exception" in value
    ]
    manifest = {
        "schema_version": 1,
        "release_candidate": "APORIA_PRODUCT_RC5",
        "status": "QUALIFIED_WITH_EXCEPTION" if qualification_exceptions else "QUALIFIED",
    ...
    print(json.dumps({"status": manifest["status"], "manifest_commitment": manifest["manifest_commitment"]}, sort_keys=True))
