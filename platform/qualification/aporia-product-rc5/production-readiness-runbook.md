...
- Soak: at least two continuous hours using the same normal authenticated WebSocket route. A user-accepted,
  narrowly scoped delta exception is classified as `QUALIFIED_WITH_EXCEPTION`, never as an unqualified
  `PASS`, and must preserve the failed long-window evidence and residual limitations.
...
The RC is qualified only if all mandatory evidence is complete and `PASS`, or if a mandatory item carries
a sealed, explicit `PASS_WITH_EXCEPTION` accepted by the user. In the latter case the whole RC is
`QUALIFIED_WITH_EXCEPTION`. No critical alert may exist, all six independent controls must fail closed,
rollback and recovery must pass, G8 must remain unused, and no tracked scientific artifact may change.
