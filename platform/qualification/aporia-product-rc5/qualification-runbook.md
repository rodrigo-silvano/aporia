# APORIA RC5 qualification execution

RC5 is a fresh full confirmation. Do not copy or resume participant or judge outputs from RC1-RC4.
...
start with `APORIA RC5 qualification`, issues 45-second read-only tickets, and archives only those owned
sessions.
...
The soak flag permitting a shorter duration is for local script validation only and is invalid normal
release evidence. Required gates are zero lost and duplicate events, unrecovered error rate below 0.5%,
all required sessions open, and measured TTFT/latency present.

An explicit user decision may accept a narrowly scoped soak exception after a long window exposes a
specific defect and the corrected path is covered by both a live targeted delta and deterministic tests.
That exception must never be represented as a normal soak pass: the evidence status is
`PASS_WITH_EXCEPTION`, the manifest status is `QUALIFIED_WITH_EXCEPTION`, and the failed long-window
result, residual limitations and exact unchanged runtime boundary must remain in the sealed evidence.
